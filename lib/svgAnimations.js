...
export function pauseSvgAnimations(svgEl, targets = ['plante', 'feuillage', 'telephone']) {
  if (!svgEl) return
  targets.forEach(id => {
    const el = svgEl.querySelector(`#${id}`)
    if (!el) return
    const elements = el.children.length > 0 ? Array.from(el.children) : [el]
    elements.forEach(e => { e.style.animationPlayState = 'paused' })
    el.style.animationPlayState = 'paused'
  })
}

export function resumeSvgAnimations(svgEl, targets = ['plante', 'feuillage', 'telephone']) {
  if (!svgEl) return
  targets.forEach(id => {
    const el = svgEl.querySelector(`#${id}`)
    if (!el) return
    const elements = el.children.length > 0 ? Array.from(el.children) : [el]
    elements.forEach(e => { e.style.animationPlayState = 'running' })
